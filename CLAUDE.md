...
- **`terraform/environments/prod/proxmox/forgejo-stack/`** — Provisions a single LXC container on Proxmox VE using the `bpg/proxmox` provider. Container gets a static IP and connects via SSH as `root` with keys loaded in ssh-agent.
- **`vagrant/vagrant-vb/forgejo-stack/`** — VirtualBox VM managed directly with Vagrant. Run `gen-inventory.sh` after `vagrant up` to generate the Ansible inventory.
- **`vagrant/vagrant-utm/forgejo-stack/`** — UTM VM (Apple Silicon) managed directly with Vagrant. Uses `utm/debian-12` box with `utm` provider block. Requires `vagrant plugin install vagrant-utm` (one-time).
...
terraform init
terraform plan
terraform apply   # Generates ansible/inventory/prod/proxmox/forgejo-stack/inventory.ini
...
- `ansible/inventory/prod/proxmox/forgejo-stack/inventory.tpl` — Terraform template generating `inventory.ini` with the container's static IP and `forgejo_domain`.
- `vagrant/<env>/forgejo-stack/gen-inventory.sh` — Shell script that reads `vagrant ssh-config` and writes the Ansible inventory. Run after `vagrant up`.
...
- Inventory path from `terraform/environments/prod/proxmox/forgejo-stack/`: 5 levels up to repo root (`../../../../../`)
- Inventory path from `vagrant/<env>/forgejo-stack/`: 3 levels up to repo root (`../../../`)
- vagrant-vb box: `cloud-image/debian-13`; vagrant-utm box: `utm/debian-12` (ARM64, no disk resize)
- Private network IP: 192.168.56.10

## Configuration

Copy `terraform/environments/prod/proxmox/forgejo-stack/terraform.tfvars.example` to `terraform.tfvars` and fill in values. Sensitive values via `TF_VAR_` environment variables.
